...
const EggDetectedController = {
    ...
    // ENDPOINT API UBAH DATA TELUR BERDASARKAN ID DOKUMEN
    async updateEggById(request, h) {
        try {
            const { eggId } = request.params; // ID dokumen telur yang akan diubah
            const { label, detectionTimestamp } = request.payload; // Data baru yang akan diubah

            const eggRef = db.collection('Egg-detected').doc(eggId);

            // Periksa apakah dokumen telur dengan eggId yang sesuai ada di dalam koleksi
            const eggDoc = await eggRef.get();

            if (!eggDoc.exists) {
                return h.response({ error: 'Telur tidak ditemukan' }).code(404);
            }
            ...
            // Lakukan pembaruan data telur berdasarkan ID dokumen
            await eggRef.update(updatedEgg);

            return {
                status: 'success',
                message: `Data telur dengan ID ${eggId} berhasil diperbarui`,
            };
        } catch (error) {
            console.error('Error:', error);
            return h.response({ error: 'Gagal memperbarui data telur' }).code(500);
        }
    },

    // ENDPOINT API HAPUS DOKUMEN BERDASARKAN ID
    async deleteEggById(request, h) {
        try {
            const { eggId } = request.params;

            // Hapus dokumen berdasarkan ID (eggId) dari koleksi Egg-detected
            await db.collection('Egg-detected').doc(eggId).delete();

            return {
                status: 'success',
                message: `Dokumen dengan ID ${eggId} pada koleksi Egg-detected berhasil dihapus`
            };
        } catch (error) {
            console.error('Error:', error);
            return h.response({ error: 'Gagal menghapus dokumen telur' }).code(500);
        }
    },
